...
(async function () {
  ...
  function productCard(p) {
    ...
    const img = p.image_url ? `<img src="${ui.esc(p.image_url)}" alt="${ui.esc(p.brand)}">` : `<span class="product__initial">${ui.esc(ui.initial(p))}</span>`;
  ...
  function entryRow(x) {
    ...
    return `<div class="entry cat-${ui.esc(x.category || "other")}" data-id="${ui.esc(x.id)}">
      <div class="entry__thumb">${ui.esc(ui.initial(x.brand ? { brand: x.brand } : { name: x.product_name }))}</div>
